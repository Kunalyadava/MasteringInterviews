...
5. ### What are the key components of Angular?
    ...
    1. **Component:**  Fundamental units in Angular that manage HTML views.
    2. **Modules:** Collections of Angular building blocks like components, directives, and services, organizing an application into logical pieces and each piece of code is called as "module" which perform a single task.
    3. **Templates:** These represent the views of an Angular application.
    4. **Services:** Reusable components that can be shared throughout the application to encapsulate and share functionality.
    5. **Metadata:**Additional data that can be added to Angular classes to provide configuration and behavior.
...
10. ### What is a module?
   ...
   as a cohesive unit for managing and scaling Angular application
    ...
    declarations: Lists the components, directives, and pipes that belong to this module. Here, AppComponent is declared, indicating that it belongs to this module and can be used within it.

    bootstrap: Specifies the root component of the application, which Angular should bootstrap when the application starts. AppComponent is set as both the root component and the bootstrap component in this example.
...
Observable
Declarative: Computation does not start until subscription, allowing it to run whenever you need the result.
Values: Provides multiple values over time.
Error Handling: Uses the subscribe method for error handling, facilitating centralized and predictable error handling.
Handling: Provides chaining and subscription to handle complex applications.

Promise 
Declarative: Executes immediately upon creation.
Values: Provides only one value.
Error Handling: Pushes errors to the child promises.
Handling: Uses only the .then() clause for chaining and handling.

  **[⬆ Back to Top](#table-of-contents)**

45. ### What is multicasting?
    Multicasting is a communication method where a single stream of 
    data is sent to multiple subscribers simultaneously. 
    This approach is efficient for scenarios where the same data 
    needs to be delivered to multiple recipients,
     reducing the need for multiple redundant transmissions.
...
subject.subscribe({...}) under the hood manages the subscriptions to
...
Disadvantages: Inefficient for transmitting data to multiple devices
Multicast
Description: One-to-many communication method
Advantages: Efficient method to transmit data to multiple devices
Disadvantages: Requires multicast support from network infrastructure
Broadcast
